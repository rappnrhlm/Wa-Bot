...
function ensureDataFiles() {
    ...
    if (!fs.existsSync(OWNER_FILE)) {
        fs.writeFileSync(
            OWNER_FILE,
            JSON.stringify([{ name: 'Raffa', number: SUPER_OWNER }], null, 4)
        );
    }
    ...
    if (!fs.existsSync(LOG_FILE)) {
        fs.writeFileSync(LOG_FILE, JSON.stringify([], null, 4));
    }
...
function isSuperOwner(number) {
    return normalizePhoneNumber(number) === SUPER_OWNER;
}
...
function logCommand(command, from, senderNumber) {
    ...
    logs.push({
        ...
        type: isGroup(from) ? 'group' : 'private',
...
// ==========================================
// SYSTEM MONITORING & MEME GENERATOR
// ==========================================
...
async function getServerStats() {
    try {
        ...
        // Penentuan Lokasi Berdasarkan Nama Device
        let city = 'Indonesia';
...
async function drawMeme(imageBuffer, topText, bottomText) {
    ...
    const fontSize = Math.floor(img.width / 11);
...
function findParticipant(metadata, jid) {
    ...
    return metadata.participants.find(p => p?.id && getJidNumber(p.id) === targetNumber);
}

async function getGroupMetadata(sock, from, msg) {
    if (!isGroup(from)) {
...
async function requireAdmin(sock, from, msg, metadata) {
    const sender = msg?.key?.participant || msg?.key?.participantAlt || from;
...
async function sendAdminMenu(sock, from, msg) {
    const metadata = await getGroupMetadata(sock, from, msg);
    if (!metadata) return;

    const admin = await requireAdmin(sock, from, msg, metadata);
    if (!admin) return;
...
async function sendHelp(sock, from, msg, command) {
    const helps = {
    ...
    const help = helps[command.toLowerCase()];
    await sock.sendMessage(
        from,
        { text: help || `❌ Help untuk !${command} tidak ditemukan.` },
        { quoted: msg }
    );
}
...
async function handleWelcome(sock, update) {
    ...
    const config = readJSON(WELCOME_FILE, {
        enabled: true,
...
async function connectToWhatsApp() {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        const senderNumber = await resolveSenderNumber(sock, msg, from);

        if (!isGroup(from) && (!senderNumber || !isOwner(senderNumber))) {
            await sock.sendMessage(
                from,
        ...
        incrementCommandStats(command);
        logCommand(command, from, senderNumber);

        // COMMAND: !ping
        if (command === 'ping') {
            await sock.sendMessage(
                from,
                { text: 'Pong! 🏓 Bot aktif lancar!' },
                { quoted: msg }
            );
            return;
        }
        ...
        if (command === 'admin-menu') {
            await sendAdminMenu(sock, from, msg);
            return;
        }

        // COMMAND: !help
        if (command === 'help') {
            await sendHelp(sock, from, msg, args[0]);
            return;
        }
...
Disk: ${server.diskUsedGB} GB / ${server.diskTotalGB} GB (${server.diskPercent}%)`;
            ...
            const serverInfo = '🖥️ *SERVER MONITOR*\n' + serverDetails;
...
╰━━━━━━━━━━━━━━━━━━━━╯`
        ...
        if (command === 'stiker' || command === 'sticker') {
            try {
                ...
                if (!imageMsg) {
                    await sock.sendMessage(
                        from,
                        { text: 'Kirim gambar dengan caption !stiker atau reply gambar.' },
                        { quoted: msg }
                    );
                    return;
                }
        ...
        if (command === 'smeme') {
            try {
                ...
                await sock.sendMessage(
                    from,
                    { text: '⏳ Sedang membuat stiker meme...' },
                    { quoted: msg }
                );
        ...
        if (command === 'brat') {
            try {
                ...
                if (!text) {
                    await sock.sendMessage(
                        from,
                        { text: 'Contoh: !brat dih so ganteng tapi emang ganteng sih' },
                        { quoted: msg }
                    );
                    return;
                }
                ...
                await sock.sendMessage(
                    from,
                    { text: '⏳ Bikin stiker Brat...' },
                    { quoted: msg }
                );

                const outputPath = `/tmp/brat-${Date.now()}-${Math.random().toString(36).slice(2)}.png`;
        ...
        // COMMAND: !owner
        if (command === 'owner') {
            if (!isSuperOwner(senderNumber)) {
                await sock.sendMessage(
                    from,
                    { text: '❌ Cuma Rapa utama yang boleh ngatur daftar owner.' },
                    { quoted: msg }
                );
                return;
            }
            ...
            if (subCommand === 'list') {
                ...
                owners.forEach(owner => {
                    ...
                    if (name === 'Mama' || name === 'Papa') return;

                    text += `${index}. ${name} — ${maskNumber(number)}`;
                    if (normalizePhoneNumber(number) === SUPER_OWNER) text += ' 👑';
                    text += '\n';
                    index++;
                });
            ...
            if (subCommand === 'add') {
                ...
                if (!number) {
                    await sock.sendMessage(
                        from,
                        { text: '❌ Contoh: !owner add 628123456789' },
                        { quoted: msg }
                    );
                    return;
                }
                ...
                const exists = owners.some(o => {
                    ...
                    return normalizePhoneNumber(ownerNumber) === number;
                });

                if (exists) {
                    await sock.sendMessage(
                        from,
                        { text: `⚠️ ${number} sudah menjadi owner.` },
                        { quoted: msg }
                    );
                    return;
                }

                owners.push({ name: args.slice(1).join(' ') || 'Owner', number });
                saveOwners(owners);

                await sock.sendMessage(
                    from,
                    { text: `✅ ${number} berhasil ditambahkan sebagai owner.` },
                    { quoted: msg }
                );
                return;
            }
            ...
            if (subCommand === 'delete' || subCommand === 'del' || subCommand === 'remove') {
                ...
                if (!number) {
                    await sock.sendMessage(
                        from,
                        { text: '❌ Contoh: !owner delete 628123456789' },
                        { quoted: msg }
                    );
                    return;
                }

                if (number === SUPER_OWNER) {
                    await sock.sendMessage(
                        from,
                        { text: '❌ Super owner tidak bisa dihapus.' },
                        { quoted: msg }
                    );
                    return;
                }
                ...
                await sock.sendMessage(
                    from,
                    { text: `✅ ${number} berhasil dihapus dari owner.` },
                    { quoted: msg }
                );
                return;
            }
        ...
        // COMMAND: !groupinfo & !listadmin
        if (command === 'groupinfo' || command === 'listadmin') {
            const metadata = await getGroupMetadata(sock, from, msg);
            if (!metadata) return;
...
!welcome off`
                ...
                writeJSON(WELCOME_FILE, config);

                await sock.sendMessage(
                    from,
                    {
                        text: `✅ Auto welcome ${config.enabled ? 'diaktifkan 🟢' : 'dimatikan 🔴'}`
                    },
                    { quoted: msg }
                );
                return;
            ...
            if (command === 'setwelcome') {
                ...
                const config = readJSON(WELCOME_FILE, { enabled: true });
                config.text = text;
                writeJSON(WELCOME_FILE, config);
            ...
            const botAdmin = await requireBotAdmin(sock, from, msg, metadata);
            if (!botAdmin) return;
            ...
            if (!target) {
                await sock.sendMessage(
                    from,
                    { text: `❌ Mention atau reply target dengan !${command}` },
                    { quoted: msg }
                );
                return;
            }

            const targetParticipant = findParticipant(metadata, target);
            if (!targetParticipant) {
                await sock.sendMessage(
                    from,
                    { text: '❌ User tidak ditemukan di grup.' },
                    { quoted: msg }
                );
                return;
            }
            ...
            try {
                if (command === 'kick') {
                    ...
                    await sock.sendMessage(
                        from,
                        {
                            text: `✅ @${getJidNumber(targetJid)} telah dikeluarkan.`,
                            mentions: [targetJid]
                        },
                        { quoted: msg }
                    );
                ...
                if (command === 'promote') {
                    ...
                    await sock.sendMessage(
                        from,
                        {
                            text: `👑 @${getJidNumber(targetJid)} sekarang menjadi admin.`,
                            mentions: [targetJid]
                        },
                        { quoted: msg }
                    );
                ...
                if (command === 'demote') {
                    ...
                    await sock.sendMessage(
                        from,
                        {
                            text: `⬇️ @${getJidNumber(targetJid)} tidak lagi menjadi admin.`,
                            mentions: [targetJid]
                        },
                        { quoted: msg }
                    );
                ...
                await sock.sendMessage(
                    from,
                    { text: `❌ Gagal menjalankan !${command}.` },
                    { quoted: msg }
                );
            }
            return;
...
(async () => {
    try {
        ...
        renderBrat = bratModule.renderBrat;

        if (!renderBrat) {
            throw new Error('renderBrat tidak ditemukan.');
        }

        console.log('Brat generator berhasil dimuat.');

        require('./web/server');
        console.log('✅ Server web sudah dipanggil');
